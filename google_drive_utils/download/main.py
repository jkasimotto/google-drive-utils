...
from google_drive_utils.service.get_service import get_service


def download(client_secret, folder_id, to=None, max=None, duplicate="skip", exclude=None, mime_type=None):
    service = get_service(
        client_secret, ['https://www.googleapis.com/auth/drive.readonly'])
    files = get_files_in_folder(service, folder_id, max)
    results = download_files(service, files, to, max,
                             duplicate, exclude, mime_type)
    return results
...
def download_files(service, files, to=None, max=None, duplicate="skip", exclude=None, mime_type=None):
    """Downloads files from a Google Drive folder.

    Args:
        service (googleapiclient.discovery.Resource): An authorized Drive API service instance.
        files (list): A list of files to download.
        to (str, optional): The local destination directory to download files to. If None, returns the downloaded files.
